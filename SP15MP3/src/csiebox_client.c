...
static void download_all(csiebox_client *client);
...
int download_file(csiebox_client *client , csiebox_protocol_meta *meta)
{
  int modi_flag = 0;
  char* homedir = (char *)malloc(sizeof(char) * PATH_MAX);
  strcpy(homedir , client->arg.path);
  printf("homedir = %s\n", homedir);
  ...
  if (lstat(req_path, &stat) < 0) {
    ...
    //fprintf(stderr , "path conflict , no need to download\n");
...
static void download_rm(csiebox_client *client , csiebox_protocol_rm *rm)
{
  char* homedir = (char *)malloc(sizeof(char) * PATH_MAX);
  strcpy(homedir , client->arg.path);
  ...
  memset(req_path, 0, PATH_MAX);
  memset(buf, 0, PATH_MAX);
  recv_message(client->conn_fd, buf, rm->message.body.pathlen);
  sprintf(req_path, "%s%s", homedir, buf);
  free(homedir);
  ...
  if ((stat.st_mode & S_IFMT) == S_IFDIR) {
    int wd = get_from_hash_by_path(&(client->inotify_hash), (void*)req_path, 0);
    inotify_rm_watch(client->inotify_fd, wd);
    char* tmp = NULL;
    del_from_hash(&(client->inotify_hash), (void**)&tmp, wd);
    free(tmp);
...
static void handle_dowload(csiebox_client* client , int *flag)
{
  csiebox_protocol_header header;
  memset(&header, 0, sizeof(header));
  if (recv_message(client->conn_fd, &header, sizeof(header)))
  {
    if (header.req.magic != CSIEBOX_PROTOCOL_MAGIC_REQ)
      return;
    if(header.req.op == CSIEBOX_PROTOCOL_OP_SYNC_META)
    {
      fprintf(stderr , "server file changed , download\n");
      csiebox_protocol_meta meta;
      if(complete_message_with_header(client->conn_fd , &header , &meta))
      {
        if(download_file(client , &meta) > 0)
          *flag = 1;
      }
    }
    else if(header.req.op == CSIEBOX_PROTOCOL_OP_RM)
    {
      fprintf(stderr , "server file removed , local rm\n");
      csiebox_protocol_rm rm;
    ...
    fprintf(stderr , "server file downloaded\n");
  }
  else
  {
...
static void monitor_home(csiebox_client* client) {
  ...
  if(client->conn_fd > client->inotify_fd)
    fdmax = client->conn_fd;
  else
    fdmax = client->inotify_fd;

  while (1) {
    read_fds = master;
    if(select(fdmax + 1 , &read_fds , NULL , NULL , NULL) == -1)
      fprintf(stderr , "select wait error\n");
    if(FD_ISSET(client->inotify_fd , &read_fds))
    {
      if(flag == 0)
      ...
      else
      {
        ...
        if ((read(client->inotify_fd, buffer, EVENT_BUF_LEN)) <= 0)
          fprintf(stderr , "there is nothing in inotify , strange\n");
        flag = 0;
      }
    }
    if(FD_ISSET(client->conn_fd , &read_fds))
      handle_dowload(client , &flag);
  }
}
...
static void add_inotify(csiebox_client* client, char* path) {
  ...
  memcpy(inotify_path, path, strlen(path) + 1);
