...
static csiebox_protocol_status sync_file_meta(csiebox_client* client, char* path , int *lkfd );
static void sync_file_data(csiebox_client* client, char* path , int *lkfd);
...
static int login(csiebox_client* client) {
  ...
  int status = 0;
  recv_message(client->conn_fd , &status , sizeof(int));
  while(status != 1)
  {
    if(status == 0)
...
static void sync_file(csiebox_client* client, char* path) {
  ...
  int lkfd = -1;
  status = sync_file_meta(client, path , &lkfd);
  if (status == CSIEBOX_PROTOCOL_STATUS_MORE) {
    sync_file_data(client, path , &lkfd);
  }
}
...
static csiebox_protocol_status sync_file_meta(csiebox_client* client, char* path , int *lkfd) {
  ...
  if ((meta.message.body.stat.st_mode & S_IFMT) == S_IFDIR) {
    ...
    *lkfd = open(path , O_RDONLY);
    if(*lkfd < 0)
      fprintf(stderr , "open err\n");
    struct flock mtlk;
    mtlk.l_type = F_RDLCK; mtlk.l_whence = SEEK_SET;
  ...
  recv_message(client->conn_fd , &blockstat , sizeof(int));
  while(blockstat != 1)
  {
    fprintf(stdout , "Server block\n");
    sleep(3);
    send_message(client->conn_fd , &blockstat , sizeof(int));
    recv_message(client->conn_fd , &blockstat , sizeof(int));
  }
  free(relative);
  ...
  if(*lkfd >= 0 && header.res.status != CSIEBOX_PROTOCOL_STATUS_MORE)
  {
    struct flock unlk;
...
static void sync_file_data(
  csiebox_client* client, char* path , int *lkfd) {
  ...
  if ((stat.st_mode & S_IFMT) == S_IFDIR) {
    ...
    if (*lkfd < 0) {
      ...
      struct flock setlk;
      setlk.l_type = F_RDLCK; setlk.l_whence = SEEK_SET;
      setlk.l_start = 0; setlk.l_len = 0; setlk.l_pid = 0;
      //if(fcntl(fd , F_SETLK , &setlk) >= 0)
        //fprintf(stderr , "lock\n");

      file.message.body.datalen = lseek(*lkfd, 0, SEEK_END);
      ...
      memset(buf, 0, 4096);
      size_t readlen;
      while ((readlen = read(*lkfd, buf, 4096)) > 0) {
        send_message(client->conn_fd, buf, readlen);
      }
      setlk.l_type = F_UNLCK;
      if(fcntl(*lkfd , F_SETLK , &setlk) >= 0)
...
static void handle_inotify(csiebox_client* client) {
  ...
  while (i < len) {
    ...
    if (event->mask & IN_CREATE) {
      ...
      int lkfd = -1;
      sync_file_meta(client, path , &lkfd);
